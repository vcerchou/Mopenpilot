...
class CarController:
  def __init__(self, dbc_name, CP, VM):
    ...
    self.standstill_hack = Params().get_bool("ToyotaStandstillHack")  # mp
  ...
  def update(self, CC, CS):
    ...
    #mp_lc
    if (CS.out.leftBlinker or CS.out.rightBlinker) and (self.CP.steeringTorqueMod == 2):
      apply_steer = 0
    ...
    if (self.frame % 3 == 0 and self.CP.openpilotLongitudinalControl) or pcm_cancel_cmd:
      ...
      if pcm_cancel_cmd and self.CP.carFingerprint in (CAR.LEXUS_IS, CAR.LEXUS_RC):
        ...
        can_sends.append(create_accel_command(self.packer, pcm_accel_cmd, pcm_cancel_cmd, self.standstill_req, lead, CS.acc_type, CS.distance)) # mp update
        self.accel = pcm_accel_cmd
      else:
        can_sends.append(create_accel_command(self.packer, 0, pcm_cancel_cmd, False, lead, CS.acc_type, CS.distance)) # mp update
