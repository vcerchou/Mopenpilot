...
from selfdrive.car.honda.values import CAR, DBC, STEER_THRESHOLD, HONDA_BOSCH, HONDA_NIDEC_ALT_SCM_MESSAGES, HONDA_BOSCH_ALT_BRAKE_SIGNAL, HONDA_COMPATIBILITY
...
def get_can_signals(CP, gearbox_msg, main_on_sig_msg):
  ...
  elif CP.carFingerprint in (CAR.ODYSSEY_CHN, CAR.CUSTOM):
  ...
  elif CP.carFingerprint in (CAR.ODYSSEY, CAR.ODYSSEY_CHN, CAR.CUSTOM):
    signals.append(("EPB_STATE", "EPB_STATUS"))
    checks.append(("EPB_STATUS", 50))
  elif CP.carFingerprint in (CAR.CLARITY, CAR.URV):
...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.hud_lead = 0
    self.mp_metric = Params().get("IsMetric")
  ...
  def update(self, cp, cp_cam, cp_body):
    ...
    elif self.CP.carFingerprint in (CAR.ODYSSEY_CHN, CAR.FREED, CAR.HRV, CAR.CUSTOM, CAR.JADE):
    ...
    if self.CP.openpilotLongitudinalControl and self.CP.carFingerprint not in (CAR.URV):
    ...
    # mp add
    if self.CP.carFingerprint in (CAR.CLARITY, CAR.URV):
      if 3. <= cp.vl["ACC_HUD"]['CRUISE_SPEED'] < 250.:
        self.cruise_setting = 2
    ...
    if self.CP.carFingerprint in (CAR.CIVIC, CAR.ODYSSEY, CAR.ODYSSEY_CHN, CAR.CRV_5G, CAR.ACCORD, CAR.ACCORDH, CAR.CIVIC_BOSCH,
                                  CAR.CIVIC_BOSCH_DIESEL, CAR.CRV_HYBRID, CAR.INSIGHT, CAR.ACURA_RDX_3G, CAR.HONDA_E, CAR.URV):
      ret.parkingBrake = cp.vl["EPB_STATUS"]["EPB_STATE"] != 0

    if self.CP.carFingerprint in HONDA_COMPATIBILITY:
  ...
  def get_can_parser(self, CP):
    ...
    if CP.carFingerprint in HONDA_COMPATIBILITY:
      return CANParser(DBC[CP.carFingerprint]["pt"], signals, [], bus_pt, False)
    else:
  ...
  def get_body_can_parser(CP):
    if CP.enableBsm in (CAR.CRV_5G, CAR.CRV_HYBRID,):
