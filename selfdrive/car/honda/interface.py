...
from selfdrive.car.mypilot_car import mp_rewrite_lateral_tuning
...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[], disable_radar=False): # pylint: disable=dangerous-default-value
    ...
    if candidate in HONDA_BOSCH:
      ...
      if candidate in (CAR.CUSTOM, CAR.URV, ):
        ret.safetyConfigs = [get_safety_config(car.CarParams.SafetyModel.hondaNidecCustom)]
      else:
    ...
    # mp add
      ...
      ret.lateralParams.torqueBP, ret.lateralParams.torqueV = [[0, 32767], [0, 32767]]  # TODO: determine if there is a dead zone at the top end
      tire_stiffness_factor = 0.82
      ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV = [[0.56], [0.16]]
      ret.wheelSpeedFactor = 1.025
    # mp end
    else:
    ...
    ret = mp_rewrite_lateral_tuning(ret) # mp add
    return ret
  ...
  def _update(self, c):
    ...
    if self.CP.pcmCruise:
      ...
      elif not ret.cruiseState.enabled and (c.actuators.accel >= 0. or not self.CP.openpilotLongitudinalControl) and self.CP.carFingerprint not in (CAR.URV,):
