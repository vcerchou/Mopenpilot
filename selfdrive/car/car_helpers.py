...
from selfdrive import global_ti  # mp add

from cereal import car
from cereal import log  # mp add
EventName = car.CarEvent.EventName
DynamicParam = log.PandaState  # mp add

from common.mypilot import CAR_FP_CORRESPONDENCE  # mp add
...
def fingerprint(logcan, sendcan):
  fixed_fingerprint = os.environ.get('FINGERPRINT', (Params().get("CarStaticFingerprint",encoding='utf8') if Params().get_bool("IsCarStaticFingerprint") else ""))
  skip_fw_query = os.environ.get('SKIP_FW_QUERY', Params().get_bool("IsCarStaticFingerprint"))
  cloudlog.info("FINGERPRINT=%s, SKIP_FW_QUERY=%s" % ( fixed_fingerprint, skip_fw_query))
...
def get_car(logcan, sendcan):
  ...
  # mp add
  if candidate in CAR_FP_CORRESPONDENCE:
    candidate = CAR_FP_CORRESPONDENCE[candidate]
  # mp end
  ...
  global_ti.saved_CarInterface = CarInterface  # mp add
