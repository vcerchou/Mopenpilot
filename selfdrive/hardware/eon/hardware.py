...
class Android(HardwareBase):
  ...
  def get_imei(self, slot):
    ...
    # mp add
    imei = parse_service_call_string(service_call(["iphonesubinfo", "3", "i32", str(slot)]))
    if len(imei) == 14:
      result = i = temp = 0;
      while (i < len(imei)):
        result += int(imei[i:i+1])
        i = i + 1
        temp = int(imei[i:i+1]) * 2
        i = i + 1
        result += temp if temp < 10 else temp - 9

      result %= 10
      result = 0 if result == 0 else 10 - result
      imei = imei + str(result)
    return imei
  ...
  def uninstall(self):
    ...
    os.system('rm -rf /data/data/com.termux/files/continue.sh /data/openpilot 2>/dev/null')
    self.reboot()
  ...
  def get_ip_addr(self):
    try:
      ...
      try:
        result = subprocess.check_output(["ifconfig", "rmnet_data0"], encoding='utf8')
        val = re.findall(r"inet addr:((\d+\.){3}\d+)", result)[0][0]
        return val
      except:
        return 'N/A'
  ...
  def get_mem_total(self):
    ...
    mem_total = round(int(matched.groups()[0])/1024/1024, 1)
    return mem_total

  def get_storage_space(self):
    statvfs = os.statvfs("/data/")
    storage_space = (statvfs.f_blocks*statvfs.f_bsize)
    return storage_space

  def get_storage_used(self, path):
    try:
      val = sum(os.path.getsize(path + f) for f in os.listdir(path) if os.path.isfile(path + f))
