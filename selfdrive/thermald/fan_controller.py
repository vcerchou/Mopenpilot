...
class BaseFanController(ABC):
  @abstractmethod
  def update(self, max_cpu_temp: float, ignition: bool, is_silence_mode: bool) -> int:
    pass
...
class EonFanController(BaseFanController):
  ...
  # 静音模式风扇转速配置
  FAN_SPEEDS_SILENCE = [0, 16384, 16384, 32768]
  # 普通模式风扇转速配置
  FAN_SPEEDS_NORMAL = FAN_SPEEDS
  ...
  def update(self, max_cpu_temp: float, ignition: bool, is_silence_mode: bool) -> int:
    self.FAN_SPEEDS = self.FAN_SPEEDS_SILENCE if is_silence_mode else self.FAN_SPEEDS_NORMAL
...
class UnoFanController(BaseFanController):
  ...
  def update(self, max_cpu_temp: float, ignition: bool, is_silence_mode: bool) -> int:
    ...
    if is_silence_mode:
      new_speed = int(interp(max_cpu_temp, [65.0, 80.0, 90], [0, 20, 60]))
