...
class PowerMonitoring:
  def __init__(self):
    ...
    # 默认保持最小关机时间可能大于mp界面设置的时间
    global MIN_ON_TIME_S
    is_mp_auto_shutdown = self.params.get_bool("IsMpAuth") and self.params.get_bool("IsAutoShutdown")
    if is_mp_auto_shutdown:
      MIN_ON_TIME_S = int(self.params.get("AutoShutdownAt")) * 60
  ...
  def should_shutdown(self, peripheralState, ignition, in_car, offroad_timestamp, started_seen):
    #mp有电池eon,没有接comma power
    if not HARDWARE.get_battery_charging() and not ignition and (offroad_timestamp is not None) and ( sec_since_boot() - offroad_timestamp) >= MAX_TIME_OFFROAD_S:
      print("eon with battery should_shutdown")
      return True
