...
def thermald_thread(end_event, hw_queue):
  ...
  # mp add
  ip_addr = 'N/A'
  mem_total = HARDWARE.get_mem_total()
  storage_space = int(HARDWARE.get_storage_space() / 1024 / 1024 / 1024)
  ...
  while not end_event.is_set():
    ...
    elif onroad_conditions["ignition"]:
    ...
    msg.deviceState.memTotal = mem_total
    msg.deviceState.storageSpace = storage_space
    msg.deviceState.isSilenceMode = is_silence_mode
    msg.deviceState.imei1 = imei1
    msg.deviceState.imei2 = imei2
    msg.deviceState.serial = serial
    # mp end
    ...
    # mp is_silence_mode, update every 10s
    if (count % int(10. / DT_TRML)) == 0:
      is_silence_mode = params.get("IsSilenceMode") == b'1'
      msg.deviceState.isSilenceMode = is_silence_mode
    # mp end

    if fan_controller is not None:
      msg.deviceState.fanSpeedPercentDesired = fan_controller.update(max_comp_temp, onroad_conditions["ignition"], is_silence_mode)
    ...
    # mp auto timeout shutdown, update every 10s
    if (count % int(10. / DT_TRML)) == 0 and off_ts is not None:
      if params.get("IsAutoShutdown") == b"1":
        timeout_s = int(params.get("AutoShutdownAt")) * 60
        now_s = sec_since_boot()
        if now_s - off_ts > timeout_s and now_s - off_ts <= MAX_TIME_OFFROAD_S:
          cloudlog.info('auto shutdown after offroad %s sec' % (now_s - off_ts) )
