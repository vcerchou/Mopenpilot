...
from selfdrive.controls.lib.following_distance import FollowingDistance
from common.params import Params

# mp停车后距离前车的距离（米）
MP_ADD_STOP_DISTANCE = float(Params().get("StoppingDistMeter"))
MP_ADD_STOP_DISTANCE = MP_ADD_STOP_DISTANCE if MP_ADD_STOP_DISTANCE >= 3.0 else 3.0
...
def get_safe_obstacle_distance(v_ego, tf=T_FOLLOW):
  return (v_ego**2) / (2 * COMFORT_BRAKE) + tf * v_ego + STOP_DISTANCE

def desired_follow_distance(v_ego, v_lead, tf=T_FOLLOW):
  return get_safe_obstacle_distance(v_ego, tf) - get_stopped_equivalence_factor(v_lead, v_ego)
...
def gen_long_model():
  ...
  desired_tf = SX.sym('desired_tf') # mp
  model.p = vertcat(a_min, a_max, x_obstacle, prev_a, desired_tf) # mp
...
def gen_long_ocp():
  ...
  desired_tf = ocp.model.p[4] # mp

  ocp.cost.yref = np.zeros((COST_DIM, ))
  ocp.cost.yref_e = np.zeros((COST_E_DIM, ))

  desired_dist_comfort = get_safe_obstacle_distance(v_ego, desired_tf) # mp
  ...
  ocp.parameter_values = np.array([-1.2, 1.2, 0.0, 0.0, T_FOLLOW]) # mp
...
class LongitudinalMpc:
  def __init__(self, e2e=False):
    self.e2e = e2e
    self.following_distance = FollowingDistance() # mp
    self.c_tf = T_FOLLOW # mp
  ...
  def get_cost_multipliers(self):
    ...
    a_change = interp(self.c_tf, TFs, [.1, .8, 1.])
    j_ego = interp(self.c_tf, TFs, [.6, .8, 1.])
    d_zone = interp(self.c_tf, TFs, [1.6, 1.3, 1.])
    return (a_change, j_ego, d_zone)

  def set_weights_for_lead_policy(self, prev_accel_constraint=True):
    # tf_s = [1.2, T_FOLLOW, 2.7]
    # x_ego_obstacle_cost_multiplier = interp(self.c_tf, tf_s, [3., 1.0, 0.1])
    # j_ego_cost_multiplier = interp(self.c_tf, tf_s, [0.5, 1.0, 1.0])
    # d_zone_cost_multiplier = interp(self.c_tf, tf_s, [4., 1.0, 1.0])
  ...
  def process_lead(self, lead):
    ...
    if lead is not None and lead.status:
      x_lead = max(0, lead.dRel - (MP_ADD_STOP_DISTANCE - 3.0))
      v_lead = lead.vLead
      a_lead = lead.aLeadK
  ...
  # mp
  def set_c_tf(self, c_tf):
    self.c_tf = clip(c_tf,  0.9, 2.7)
    self.set_weights()
  ...
  def update(self, carstate, radarstate, v_cruise):
    ...
    # mp add
    tf = self.following_distance.update_TF(carstate,T_FOLLOW)
    if self.c_tf != tf:
      self.set_c_tf(tf)
      print("===update tf=%s" % self.c_tf)
    # mp end
    ...
    v_cruise_clipped = np.clip(v_cruise * np.ones(N+1),
                               v_lower,
                               v_upper)
    cruise_obstacle = np.cumsum(T_DIFFS * v_cruise_clipped) + get_safe_obstacle_distance(v_cruise_clipped, self.c_tf)

    x_obstacles = np.column_stack([lead_0_obstacle, lead_1_obstacle, cruise_obstacle])
    self.source = SOURCES[np.argmin(x_obstacles[0])]
    self.params[:,2] = np.min(x_obstacles, axis=1)
    self.params[:,3] = np.copy(self.prev_a)
    self.params[:,4] = self.c_tf # mp add
