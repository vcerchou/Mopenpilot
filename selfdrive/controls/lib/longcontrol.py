...
class LongControl:
  ...
  def update(self, active, CS, long_plan, accel_limits, t_since_plan):
    ...
    if len(speeds) == CONTROL_N:
      ...
      #mp，tssp起步油门迟滞会导致pid超调,考虑适当调大v_target
      if v_target_future > v_target:
        offset = interp(v_target, [0., 3.5, 6], [0.6, 0.6, 0.15])
        v_target = interp(offset + t_since_plan, T_IDXS[:CONTROL_N], speeds)
    else:
      v_target = 0.0
      v_target_future = 0.0
      a_target = 0.0
