...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None, CI=None):
    ...
    if self.sm is None:
      ...
      self.sm = messaging.SubMaster(['deviceState', 'pandaStates', 'peripheralState', 'modelV2', 'liveCalibration',
                                     ...
                                     'managerState', 'liveParameters', 'radarState', 'trafficAssist'] + self.camera_packets + joystick_packet,
                                     ignore_alive=ignore, ignore_avg_freq=['radarState', 'longitudinalPlan'])

    # set alternative experiences from parameters # mp update
    self.disengage_on_accelerator = not params.get_bool("DisengageOnAccelerator")
    self.CP.alternativeExperience = 9
    ...
    # mp add
    self.limit_speed_ctrl = False
    self.limit_speed_ctrl_last = False
    self.mp_params = params
    self.is_sr_offset = self.mp_params.get_bool("IsSteerRatioOffset")
    self.sr_offset = float(self.mp_params.get("SteerRatioOffset"))
    # mp end
  ...
  def update_events(self, CS):
    ...
    if self.sm['deviceState'].thermalStatus >= ThermalStatus.red:
      if self.sm['deviceState'].ambientTempC >= 0:
        self.events.add(EventName.overheat)
      else:
        self.events.add(EventName.overcool)
    ...
    for i, pandaState in enumerate(self.sm['pandaStates']):
      ...
      if pandaState.torqueInterceptorDetected and not self.ti_ready: # mp add ti
        self.ti_ready = True
  ...
  def state_transition(self, CS):
    ...
    # mp trafficAssist add
    self.limit_speed_ctrl_last = self.limit_speed_ctrl
    self.limit_speed_ctrl = False
    if self.sm['trafficAssist'].state != 0:
      self.events.add(EventName.inSpeedLimitArea)
      if self.sm['trafficAssist'].state == 2:
        self.v_cruise_kph = self.sm['trafficAssist'].speedLimit
        self.limit_speed_ctrl = True
    if self.limit_speed_ctrl_last and not self.limit_speed_ctrl:
      self.events.add(EventName.outSpeedLimitArea)
    # mp trafficAssist end
  ...
  def state_control(self, CS):
    ...
    if (self.sm.frame % int(2. / DT_CTRL) == 0):
      self.is_sr_offset = self.mp_params.get_bool("IsSteerRatioOffset")
      self.sr_offset = float(self.mp_params.get("SteerRatioOffset"))
    if self.is_sr_offset:
      sr = max(sr + self.sr_offset, self.CP.steerRatio - 6.0)
    # mp end
    ...
    if not self.joystick_mode:
      ...
      #mp no accel when vRel< -5.0
      if actuators.accel > 0. and CS.vEgo > 10.0 and self.sm['longitudinalPlan'].hasLead and self.sm['radarState'].leadOne.vRel <=-5.0:
        actuators.accel = 0.
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    controlsState.steeringAngleDesiredDeg = CC.actuators.steeringAngleDeg # mp add
