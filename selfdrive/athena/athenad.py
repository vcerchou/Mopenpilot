...
ATHENA_HOST = os.getenv('ATHENA_HOST', 'wss://mypilot.com.cn')
...
def getVersion() -> Dict[str, str]:
  # mp add
  version = get_version()
  traffic_assist_auth = Params().get_bool("TrafficAssistAuth")
  if traffic_assist_auth:
    version += "-SL"
  version += "-NK"
  ...
  return {
    "version": version,  # mp update
    "remote": get_origin(''),
    "branch": get_short_branch(''),
    "commit": get_commit(default=''),
    "device": device  # mp add
  }
...
@dispatcher.add_method
def setSign(sign):
  Params().put("MyPilotKey", sign)
  return sign


@dispatcher.add_method
def setParams(key, val):
  res = 0
...
@dispatcher.add_method
def setIsUpgradeVersion(val):
  Params().put("IsUpgradeVersion", val)
  return val


@dispatcher.add_method
def getUniqueCode():
  return {"imei1": HARDWARE.get_imei(0), "imei2": HARDWARE.get_imei(1), "serial": HARDWARE.get_serial()}
