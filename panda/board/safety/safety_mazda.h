...
#define MAZDA_LKAS2         0x249  // mp add
#define MAZDA_LKAS_HUD      0x440
#define MAZDA_CRZ_CTRL      0x21c
#define MAZDA_CRZ_BTNS      0x09d
#define TI_STEER_TORQUE     0x24A  // mp add
...
const CanMsg MAZDA_TX_MSGS[] = {{MAZDA_LKAS, 0, 8}, {MAZDA_CRZ_BTNS, 0, 8}, {MAZDA_LKAS2, 1, 8}, {MAZDA_LKAS_HUD, 0, 8}}; // mp update
...
static int mazda_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && ((int)GET_BUS(to_push) == MAZDA_MAIN)) {
    ...
    if (!torque_interceptor_detected) {  // mp add
