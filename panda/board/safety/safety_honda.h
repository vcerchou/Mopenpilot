...
// mp add
static const addr_checks* honda_nidec_custom_init(uint16_t param) {
  is_honda_custom = true;
  gas_interceptor_detected = 0;
  honda_hw = HONDA_NIDEC;
  honda_alt_brake_msg = false;
  honda_bosch_long = false;
...
// mp add
static int honda_nidec_custom_fwd_hook(int bus_num, CANPacket_t *to_fwd) {
  // fwd from car to camera. also fwd certain msgs from camera to car
  // 0xE4 is steering on all cars except CRV and RDX, 0x194 for CRV and RDX,
  // 0x1FA is brake control, 0x30C is acc hud, 0x33D is lkas hud,
  int bus_fwd = -1;
  ...
  if (!relay_malfunction) {
    ...
    if (bus_num == 2) {
      ...
      bool is_lkas_msg = (addr == 0xE4) || (addr == 0x194) || (addr == 0x33D);
      bool is_acc_hud_msg = addr == 0x30C;
      bool block_fwd = is_lkas_msg || is_acc_hud_msg;
      if (!block_fwd) {
        bus_fwd = 0;
      }
    }
...
const safety_hooks honda_nidec_custom_hooks = {
  .init = honda_nidec_custom_init,
  .rx = honda_rx_hook,
  .tx = honda_tx_hook,
  .tx_lin = nooutput_tx_lin_hook,
  .fwd = honda_nidec_custom_fwd_hook,
};
